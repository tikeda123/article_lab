...
このラボは、良く見えるバックテストをどのように疑うかを学ぶための公開学習ログでもある。実験成果物は、選択プロセス、前提、限界を見える形にするために共有しており、選ばれた戦略の頑健性を主張するためのものではない。

共有している出力ファイル、スクリプトのロジック、または記事本文に基づいて、誤り、再現性の問題、簡易PBO設計への疑問、別の解釈があれば指摘していただけるとありがたい。
