...
このラボは、再現可能なFX実験を通じてクオンツ分析の考え方を学ぶための公開学習ログでもある。ここで示す表や図は、リターン分布分析についての最終結論ではなく、前提と観察結果を後から検証・修正できるようにするための材料である。

共有しているCSV、スクリプト、生成出力、または記事本文に基づいて、誤り、別の読み方、新しい分析観点があれば指摘していただけるとありがたい。
...
記事対応の2022年以降版を再生成する場合は、`lab_1` ディレクトリで次を実行する。
