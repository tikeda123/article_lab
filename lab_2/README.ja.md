...
このディレクトリは、Qiita記事「[クオンツトレーダーは『買われすぎ・売られすぎ』をどう判断するのか](https://qiita.com/tikeda123/items/8dfcc1c09e34d5304d49)」の根拠データ、検証スクリプト、記事用図表出力をまとめた実験ラボである。
...
このラボは、主観的な相場表現を検証可能な条件へ変換する過程を記録する公開学習ログでもある。定義、前提、結果を見える形で残し、後から検証・修正・改善できるようにすることを重視している。

共有しているAUDJPYデータ、実験スクリプト、生成出力、または記事本文に基づいて、誤り、再現性の問題、別の解釈、新しい視点があれば指摘していただけるとありがたい。
